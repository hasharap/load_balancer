/*
file: Rak3172_ChSide_FirmUpdate.js
 * Update the firmware of rak3172 device at the charger side from SOM
 * Date : Nov 07 2025
...
async function main(){
	...
	getFileSize(binLocation)
		...
		.then( async msg =>{
            ...
            console.log(colourize("1 HIGH BOOT Pin,",colour.green));
            
            bootPin = new GPIO(BOOTPIN,'out',0);
			console.log("BOOTPIN SET and LOW");
		...
		.then( msg =>{ 
			...
			return send(
				colourize("3 EXTENDED ERASE CMD (Changed) ",colour.green),
				CMD_DELAY,
				Buffer.from([0x44,0xBB]))
		...
		.then (msg =>{
			/*Flash Erasing*/
			return send(
				colourize("4 GLOBAL ERASE REQ (Changed) ",colour.green),
				CMD_DELAY,
        ...
        .then(async (msg)=>{
			/*RESET BOOTMODE*/
            console.log(colourize("5 LOW BOOT Pin,",colour.green));
